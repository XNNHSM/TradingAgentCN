...
export class AgentConfigService {
  ...
  getLLMConfig() {
    return {
      // 主要提供商
      primaryProvider: this.configService.get<string>('LLM_PRIMARY_PROVIDER', 'dashscope'),
      
      // 百炼配置
      dashscope: {
        apiKey: this.configService.get<string>('DASHSCOPE_API_KEY'),
        baseUrl: this.configService.get<string>('DASHSCOPE_BASE_URL', 'https://dashscope.aliyuncs.com/api/v1/services/aigc/text-generation/generation'),
        models: {
          fast: this.configService.get<string>('DASHSCOPE_FAST_MODEL', 'qwen-turbo'),
          standard: this.configService.get<string>('DASHSCOPE_STANDARD_MODEL', 'qwen-plus'),
          premium: this.configService.get<string>('DASHSCOPE_PREMIUM_MODEL', 'qwen-max'),
          longContext: this.configService.get<string>('DASHSCOPE_LONG_CONTEXT_MODEL', 'qwen-max-longcontext'),
        },
      },
      ...
      openai: {
        ...
        models: {
          fast: this.configService.get<string>('OPENAI_FAST_MODEL', 'gpt-3.5-turbo'),
          standard: this.configService.get<string>('OPENAI_STANDARD_MODEL', 'gpt-4'),
          premium: this.configService.get<string>('OPENAI_PREMIUM_MODEL', 'gpt-4-turbo'),
        },
      },
      ...
      google: {
        ...
        baseUrl: this.configService.get<string>('GOOGLE_BASE_URL', 'https://generativelanguage.googleapis.com/v1beta'),
        models: {
          fast: this.configService.get<string>('GOOGLE_FAST_MODEL', 'gemini-pro'),
          standard: this.configService.get<string>('GOOGLE_STANDARD_MODEL', 'gemini-pro'),
          premium: this.configService.get<string>('GOOGLE_PREMIUM_MODEL', 'gemini-pro'),
        },
      },
      ...
      defaults: {
        ...
        temperature: this.configService.get<number>('LLM_DEFAULT_TEMPERATURE', 0.7),
        maxTokens: this.configService.get<number>('LLM_DEFAULT_MAX_TOKENS', 2000),
      },
    };
  }
   ...
   * 构建智能体配置
   */
  private buildAgentConfig(agentType: string, defaults: Partial<AgentConfig>): AgentConfig {
    const llmDefaults = this.getLLMConfig().defaults;
    
    return {
      model: this.configService.get<string>(`${agentType}_MODEL`, defaults.model || llmDefaults.temperature.toString()),
      temperature: this.configService.get<number>(`${agentType}_TEMPERATURE`, defaults.temperature || llmDefaults.temperature),
      maxTokens: this.configService.get<number>(`${agentType}_MAX_TOKENS`, defaults.maxTokens || llmDefaults.maxTokens),
      timeout: this.configService.get<number>(`${agentType}_TIMEOUT`, defaults.timeout || llmDefaults.timeout),
      retryCount: this.configService.get<number>(`${agentType}_RETRY_COUNT`, defaults.retryCount || llmDefaults.maxRetries),
  ...
  getDataSourceConfig() {
    return {
      ...
      // 财务数据
      financialData: {
        provider: this.configService.get<string>('FINANCIAL_DATA_PROVIDER', 'mock'),
  ...
  getSystemConfig() {
    return {
      // 并发控制
      maxConcurrentAnalysts: this.configService.get<number>('MAX_CONCURRENT_ANALYSTS', 3),
      maxConcurrentResearchers: this.configService.get<number>('MAX_CONCURRENT_RESEARCHERS', 2),
      maxConcurrentTraders: this.configService.get<number>('MAX_CONCURRENT_TRADERS', 2),
      ...
      enableAgentLogs: this.configService.get<boolean>('ENABLE_AGENT_LOGS', true),
  ...
  validateConfig(): { isValid: boolean; errors: string[] } {
    ...
    // 检查主要LLM提供商配置
    if (llmConfig.primaryProvider === 'dashscope' && !llmConfig.dashscope.apiKey) {
      errors.push('DASHSCOPE_API_KEY is required when using dashscope as primary provider');
    }
    ...
    if (llmConfig.primaryProvider === 'google' && !llmConfig.google.apiKey) {
      errors.push('GOOGLE_API_KEY is required when using google as primary provider');
    }

    // 检查数据库配置
    const requiredDbConfig = ['DB_HOST', 'DB_PORT', 'DB_USERNAME', 'DB_DATABASE'];
