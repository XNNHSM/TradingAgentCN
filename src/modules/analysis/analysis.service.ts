...
import { AgentOrchestratorService, AnalysisResult, QuickAnalysisResult } from '../../agents/services/agent-orchestrator.service';
import { AgentContext } from '../../agents/interfaces/agent.interface';
import { AnalysisRecord } from './entities/analysis-record.entity';
import { CreateAnalysisDto, GetAnalysisRecordsDto } from './dto/create-analysis.dto';
...
export class AnalysisService {
   ...
   * 创建新的分析任务
   */
  async createAnalysis(dto: CreateAnalysisDto): Promise<Result<AnalysisRecord>> {
    ...
    try {
      ...
      try {
        ...
        if (dto.analysisType === 'full') {
          analysisResult = await this.agentOrchestrator.executeFullAnalysis(context);
        } else {
          analysisResult = await this.agentOrchestrator.executeQuickAnalysis(context);
        }
      } catch (error) {
   ...
   * 获取分析记录列表
   */
  async getAnalysisRecords(dto: GetAnalysisRecordsDto): Promise<Result<PaginatedResult<AnalysisRecord>>> {
    try {
      const queryBuilder = this.analysisRepository.createQueryBuilder('analysis');

      // 添加查询条件
      if (dto.stockCode) {
        queryBuilder.andWhere('analysis.stockCode = :stockCode', { stockCode: dto.stockCode });
      }

      if (dto.analysisType) {
        queryBuilder.andWhere('analysis.analysisType = :analysisType', { analysisType: dto.analysisType });
      }
      ...
      const paginatedResult = new PaginatedResult(records, total, dto.page, dto.limit);
   ...
   * 获取股票的最新分析
   */
  async getLatestAnalysis(stockCode: string): Promise<Result<AnalysisRecord | null>> {
    try {
      const record = await this.analysisRepository.findOne({
        where: { stockCode },
      ...
      return Result.success(record, record ? '获取最新分析成功' : '暂无分析记录');
  ...
  async getAnalysisStats(): Promise<Result<any>> {
    try {
      ...
      const successCount = await this.analysisRepository.count({ where: { status: 'success' } });
      const failedCount = await this.analysisRepository.count({ where: { status: 'failed' } });
      ...
      const stats = {
        ...
        averageExecutionTime: avgExecutionTime?.avg ? Math.round(avgExecutionTime.avg) + 'ms' : '0ms',
      };
   ...
   * 构建时间范围
   */
  private buildTimeRange(startDate?: string, endDate?: string): { startDate: Date; endDate: Date } {
    const end = endDate ? new Date(endDate) : new Date();
    const start = startDate ? new Date(startDate) : new Date(end.getTime() - 30 * 24 * 60 * 60 * 1000); // 默认30天前
   ...
   * 提取平均评分
   */
  private extractAverageScore(result: AnalysisResult | QuickAnalysisResult): number | undefined {
   ...
   * 提取最终建议
   */
  private extractFinalRecommendation(result: AnalysisResult | QuickAnalysisResult): any {
    if ('summary' in result) {
   ...
   * 提取置信度
   */
  private extractConfidence(result: AnalysisResult | QuickAnalysisResult): number | undefined {
    if ('summary' in result) {
   ...
   * 提取关键洞察
   */
  private extractKeyInsights(result: AnalysisResult | QuickAnalysisResult): string[] | undefined {
    if ('summary' in result) {
   ...
   * 提取主要风险
   */
  private extractMajorRisks(result: AnalysisResult | QuickAnalysisResult): string[] | undefined {
    if ('summary' in result) {
