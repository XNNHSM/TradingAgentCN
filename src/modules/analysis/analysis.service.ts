...
import { WorkflowExecutionMode } from "../../agents/langgraph/integration/workflow-bridge";
...
export class AnalysisService {
  ...
  constructor(
    ...
    this.logger.log(`AnalysisService initialized - hybridWorkflowService: ${this.hybridWorkflowService ? 'available' : 'NOT available'}`);
  }
  ...
  ): Promise<Result<{ workflowId?: string; sessionId?: string; message: string; existingAnalysis?: boolean; executionMode?: string }>> {
    ...
    try {
      ...
      if (todayAnalysis) {
        ...
        return Result.success({
          ...messageResult.data,
          executionMode: 'existing_analysis'
      ...
      // 检查是否启用混合工作流
      const enableHybridWorkflow = this.configService?.get<boolean>('ANALYSIS_ENABLE_HYBRID_WORKFLOW', false);
      
      if (enableHybridWorkflow && this.hybridWorkflowService) {
        return await this.executeHybridAnalysis(dto, sessionId);
      } else {
        return await this.executeTraditionalAnalysis(dto, sessionId);
      }
  ...
  private async executeHybridAnalysis(
    ...
    this.logger.log(`使用混合工作流分析股票: ${dto.stockCode}`);

    try {
      // 使用混合工作流服务执行分析
      const result = await this.hybridWorkflowService!.executeHybridWorkflow({
        ...
        realtimeRequired: dto.realtimeRequired || false,
      });

      if (result.success && result.data) {
        const response = {
          workflowId: result.data.workflowId || sessionId,
          sessionId,
          message: `股票 ${dto.stockCode} 的混合工作流分析已启动，执行模式: ${result.mode}`,
          existingAnalysis: false,
          executionMode: result.mode,
        };

        this.logger.log(`混合工作流分析已启动: ${result.mode}`);
        return Result.success(response, "混合工作流分析已启动");
      } else {
        return Result.error(result.error || '混合工作流分析失败');
      }

    } catch (error) {
      this.logger.error(`混合工作流分析失败: ${error.message}`, error.stack);
      
      // 如果混合工作流失败，回退到传统Temporal工作流
      this.logger.log('混合工作流失败，回退到传统Temporal工作流');
      return await this.executeTraditionalAnalysis(dto, sessionId);
    }
  }
  ...
  private async executeTraditionalAnalysis(
    ...
    this.logger.log(`使用传统Temporal工作流分析股票: ${dto.stockCode}`);
    ...
    try {
      ...
      this.logger.log(`传统Temporal工作流已启动: ${workflowHandle.workflowId}`);
      return Result.success(response, "分析工作流已启动");

    } catch (error) {
      this.logger.error(`传统Temporal工作流分析失败: ${error.message}`, error.stack);
      throw new BadRequestException(`创建分析任务失败: ${error.message}`);
    }
  }
  ...
  async createEnhancedAnalysis(
    dto: CreateAnalysisDto,
  ): Promise<Result<{ workflowId?: string; sessionId?: string; message: string; analysisType?: string; existingAnalysis?: boolean; executionMode?: string }>> {
    this.logger.log(`开始增强版分析股票: ${dto.stockCode}`);
    ...
    try {
      ...
      if (enableHybridWorkflow && this.hybridWorkflowService) {
        ...
        const traditionalResult = await this.executeTraditionalAnalysis(dto, sessionId);
        return Result.success({
          ...traditionalResult.data!,
          analysisType: 'comprehensive',
        }, traditionalResult.message);
      }
