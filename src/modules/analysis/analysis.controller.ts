...
import { CreateAnalysisDto, GetAnalysisRecordsDto, GetAnalysisDetailDto } from './dto/create-analysis.dto';
...
export class AnalysisController {
  ...
  async createAnalysis(@Body() dto: CreateAnalysisDto): Promise<Result<AnalysisRecord>> {
  ...
  async getAnalysisRecords(@Body() dto: GetAnalysisRecordsDto): Promise<Result<PaginatedResult<AnalysisRecord>>> {
  ...
  async getAnalysisDetail(@Body() dto: GetAnalysisDetailDto): Promise<Result<AnalysisRecord>> {
  ...
  @ApiResponse({ status: 404, description: '分析记录不存在' })
  async deleteAnalysis(@Body() dto: GetAnalysisDetailDto): Promise<Result<null>> {
  ...
  async getLatestAnalysis(@Body() body: { stockCode: string }): Promise<Result<AnalysisRecord | null>> {
  ...
  async quickAnalysis(@Body() body: { stockCode: string; stockName?: string }): Promise<Result<AnalysisRecord>> {
  ...
  async fullAnalysis(@Body() body: { stockCode: string; stockName?: string }): Promise<Result<AnalysisRecord>> {
    const dto: CreateAnalysisDto = {
      stockCode: body.stockCode,
      stockName: body.stockName,
      analysisType: 'full',
