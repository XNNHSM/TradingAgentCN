...
export class AgentOrchestratorService {
  ...
  async executeFullAnalysis(context: AgentContext): Promise<AnalysisResult> {
    ...
    try {
      ...
      // 第二阶段：研究员基于分析师结果进行研究
      const researchResults = await this.runResearchPhase(context, analystResults);
      
      // 第三阶段：交易员基于所有结果做决策
      const tradingResults = await this.runTradingPhase(context, [...analystResults, ...researchResults]);
      
      // 第四阶段：反思智能体进行质量控制
      const reflectionResult = await this.runReflectionPhase(context, [...analystResults, ...researchResults, ...tradingResults]);
      ...
      return {
        ...
        tradingResults,
        reflectionResult,
        summary: this.generateSummary([...analystResults, ...researchResults, ...tradingResults], reflectionResult),
   ...
   * 第一阶段：运行分析师团队
   */
  private async runAnalystsPhase(context: AgentContext): Promise<AgentResult[]> {
   ...
   * 第二阶段：运行研究员团队
   */
  private async runResearchPhase(context: AgentContext, analystResults: AgentResult[]): Promise<AgentResult[]> {
   ...
   * 第三阶段：运行交易员团队
   */
  private async runTradingPhase(context: AgentContext, previousResults: AgentResult[]): Promise<AgentResult[]> {
   ...
   * 第四阶段：运行反思智能体
   */
  private async runReflectionPhase(context: AgentContext, allResults: AgentResult[]): Promise<AgentResult> {
   ...
   * 生成分析摘要
   */
  private generateSummary(allResults: AgentResult[], reflectionResult: AgentResult): AnalysisSummary {
    const scores = allResults.filter(r => r.score).map(r => r.score!);
    const recommendations = allResults.filter(r => r.recommendation).map(r => r.recommendation!);
    ...
    // 确定主导建议
    const dominantRecommendation = Object.keys(recommendationCounts).reduce((a, b) => 
    ...
    return {
      ...
      finalRecommendation: reflectionResult.recommendation || dominantRecommendation as TradingRecommendation,
   ...
   * 快速分析 - 仅运行核心分析师
   */
  async executeQuickAnalysis(context: AgentContext): Promise<QuickAnalysisResult> {
    ...
    try {
      ...
      const tradingResult = await this.conservativeTrader.analyze(tradingContext);
   ...
   * 生成快速分析摘要
   */
  private generateQuickSummary(coreResults: AgentResult[], tradingResult: AgentResult): QuickSummary {
    ...
    return {
      averageScore: Math.round(avgScore * 10) / 10,
      recommendation: tradingResult.recommendation || TradingRecommendation.HOLD,
